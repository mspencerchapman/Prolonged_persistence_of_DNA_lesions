...
# Step 1: Import the 3 phylogenies ----------------------------------------
...
captured_lesion_set_list<-Map(nsamp=tree_sizes,nPVVs=nPVVs_in_data,pop_number=trees_to_use,simpop=simulated_populations,lesion_set=lesion_info_list,function(nsamp,nPVVs,pop_number,simpop,lesion_set) {
  ...
  for(k in 1:length(lesion_info_PVVs)) {
    if(k%%100==0) {cat(paste(k, "lesions assessed.",n_detected_PVV_counter,"PVVs captured so far."),sep="\n")}
    ...
    if(any(subclades_detected)) {
      ...
      if(length(list$captured_pure_subclades)<=2) {
        ...
        if(list$detected_as_PVV) {n_detected_PVV_counter<-n_detected_PVV_counter+1; cat(paste(n_detected_PVV_counter,"of",n_target_PVVs,"detected"),sep="\n")}
      }
    ...
    if(n_detected_PVV_counter==n_target_PVVs) {cat("Reached the target number of PVVs",sep="\n");break}
  }
  
  if(n_detected_PVV_counter<n_target_PVVs) {cat("All lesions assessed and target number of PVVs not reached.",sep="\n")}
  ...
  cat(paste("Total recorded number of captured PVVs:",length(captured_lesion_info_PVVs)),sep="\n")
