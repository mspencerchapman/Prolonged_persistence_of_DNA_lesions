...
##--------Now import the data from the posterior predictive checks ---------------------------------

setwd(ABC_dir)
PPC_files=list.files(path=paste0(ABC_dir,"PPCs2"),full.names = T)
PPC_sim_res_file<-"extracted_lesion_durations_PPC2.Rds"

if(file.exists(PPC_sim_res_file)) {
  PPC_sim_res<-readRDS(PPC_sim_res_file)
} else {
  cat(paste("Extracting information from",length(PPC_files),"files"),sep="\n")
  PPC_sim_res<-lapply(PPC_files,function(file) {
    cat(file,sep="\n")
    x<-readRDS(file)
    temp=lapply(x, function (individual) {
      simpop<-readRDS(paste0(simpop_dir,"/simulated_populations_",individual$pop_number,".Rds"))
  ...
  saveRDS(PPC_sim_res,file = PPC_sim_res_file)
}


##Get the mean/ dispersion coeffecients from these mutation sets
all_PPC_res<-lapply(PPC_sim_res,function(list) {
...
##--------Now visualize the summary stats and compare to the data ---------------------------------
library(ggplot2)
...
plot.ppcsumstats<-all_PPC_res%>%
  ...
  scale_y_continuous(limits=c(0.1,2.25))+
  my_theme+
  labs(x="Mean MMLD", y = "Estimated dispersion of MMLD distribution")

plot.allsumstats<-all_res%>%
  ggplot(aes(x=mean,y=disp))+
  #geom_bin2d(bins = 30) +
  #scale_fill_continuous(type = "viridis") +
  stat_density_2d(aes(fill = ..level..), geom = "polygon", colour="white",linewidth=0.2)+
...
comb_plots<-gridExtra::arrangeGrob(grobs = list(plot.allsumstats,plot.ppcsumstats),nrow=1,widths = c(1,1))
ggsave(filename=paste0(plots_dir,"PPC_plots.pdf"),comb_plots,width=7, height=2.5)
