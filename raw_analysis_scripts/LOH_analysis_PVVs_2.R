...
Base_counts_results_PVVs=lapply(data_sets, function(dataset) {
  phasing_output_dir=paste0("/lustre/scratch119/casm/team154pc/ms56/lesion_segregation/phasing_output2/",dataset)
  ...
  data_set_out=lapply(Sample_IDs,function(sample) {
    ...
    sample_out=lapply(1:nrow(data_set_sample_PVVs),function(i) {
      ...
      pure_subclade_base_counts=get_clade_base_counts(pure_subclades,tree=tree_curr,Chrom=Chrom,Pos=Pos,project=sample_info$project,ref_sample_set=Ref_sample_set,phasing_output_dir=phasing_output_dir,distance=2000,force_rerun=F)
      #Confirm those that are truly heterozygous in the positive subclades - this may include the mutation itself
      positive_subclades=which(names(pure_subclades)=="pure_positive")
      het_positions<-return_heterozygous_SNPs(base_counts_list=pure_subclade_base_counts[positive_subclades])
      het_positions<-het_positions[het_positions!=Pos] #Exclude the position of the actual mutation
      
      #if((all(is.na(het_positions))|length(het_positions)==0) & Chrom!="Y") { #Don't bother re-running for Y, as will always be homozygous
      #  print("Re-running the base counts script with longer distance of 5000")
      #  pure_subclade_base_counts=get_clade_base_counts(pure_subclades,tree=tree_curr,Chrom=Chrom,Pos=Pos,project=sample_info$project,ref_sample_set=Ref_sample_set,phasing_output_dir=phasing_output_dir,distance=5000,force_rerun=T)
      #  #Confirm those that are truly heterozygous in the positive subclades - this may include the mutation itself
      #  het_positions<-return_heterozygous_SNPs(base_counts_list=pure_subclade_base_counts[positive_subclades])
      #  het_positions<-het_positions[het_positions!=Pos] #Exclude the position of the actual mutation
      #}
      
      if(all(is.na(het_positions))|length(het_positions)==0) {
        result<-NA
        } else {
        #Now assess if remain heterozygous in the negative subclades
        negative_subclades=which(names(pure_subclades)=="pure_negative")
        neg_het_positions<-return_heterozygous_SNPs(base_counts_list=pure_subclade_base_counts[negative_subclades])
        
        #Compare the heterozygous positions from the negative and positive clades to see if there has been LOH in the negative
        LOH_results=function(neg_het_positions,pos_het_positions) {if(all(pos_het_positions%in%neg_het_positions)) {return("No LOH")} else if(!any(pos_het_positions%in%neg_het_positions)) {return("LOH")} else {return("Conflicting")}}
        res_neg=LOH_results(neg_het_positions=neg_het_positions,pos_het_positions = het_positions)
        print(res_neg); return(res_neg)
      }
      print(result)
      return(list(result=result,pure_subclade_base_counts=pure_subclade_base_counts))
    })
    return(sample_out)
  })
  return(data_set_out)
...
Base_counts_results_PVVs=unlist(Base_counts_results_PVVs,recursive=F)
Base_counts_results_PVVs=unlist(Base_counts_results_PVVs,recursive=F)
names(Base_counts_results_PVVs)<-mutations%>%filter(Type=="PVV")%>%pull(Chrom_pos)

LOH_result<-sapply(Base_counts_results_PVVs,function(list) return(list$result))





save(Base_counts_results_PVVs,"output2/Base_counts_results_PVVs_all")
