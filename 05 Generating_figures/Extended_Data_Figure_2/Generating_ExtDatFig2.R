...
#========================================#
# CREATE THE SUMMARY PLOTS: FAIL MAVs ####
#========================================#
...
## Generate Extended Data Figures 2a,b ----
for(ID in c("KX003_5_01","KX008_2_01")) {
  ...
  individual_MAVs<-mutations%>%
    filter(Class=="FAIL" & Sample_ID==ID & Type=="MAV")%>%
  ...
  if(nrow(individual_MAVs>0)) {
    ...
    pdf(paste0(plots_dir,"Fail_MAV_summary_",ID,".pdf"),width=7,height=9)
    tree=plot_tree(tree = tree,cex.label = 0,lwd=ifelse(ID=="KX004_5_01",0.25,0.5),vspace.reserve = 1.8,cex.axis=0.5,tck=-0.01)
