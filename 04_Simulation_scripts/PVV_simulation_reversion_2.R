...
all_PVV_plots_list=lapply(data_sets,function(dataset) {
  ...
  data_set_out=lapply(Sample_IDs,function(sample) {
    ...
    PVV_data$result<-ifelse(is.na(PVV_data$lesion_node),"FAIL","PASS")
    ...
    PVV_data$by_chance_prob=sapply(1:nrow(PVV_data),function(i) {
      ...
      pure_negative=pure_subclades[which(names(pure_subclades)=="pure_negative")]
      by_chance=prod(sapply(c(lesion_node,pure_negative),function(node) if(node==tree$edge[1,1]){return(1)}else{return(tree$edge.length[tree$edge[,2]==node]/sum(tree$edge.length))}))
    ...
    #Total number of times to simulate a mutation occurring twice independently
    ...
    rownames(sim$NV)=rownames(sim$NR)<-1:nrow(sim$NV)
    sim$details=data.frame(mut_ref=as.character(1:nsim),node=NA,pval=NA)
    sim$sim_nodes<-data.frame(mutation_node=rep(NA,nsim),reversion_node=rep(NA,nsim))
    sim$isPVV<-rep(NA,nsim)
    mean_depth=mean(rowMeans(filtered_muts$COMB_mats.tree.build$NR))
    
    #Calculate the total branch lengths of the biggest clade - use this to normalize the probabilities of somatic reversion occurring by chance
    get_total_clade_branch_lengths=function(node,tree) {return(sum(tree$edge.length[tree$edge[,2]%in%get_all_node_children(node,tree)]))}

    #Simulate read counts
    #The probability of a branch being affected ~ n_mutations_on_branch x sum(branch_lengths[within clade])
    internal_nodes=tree$edge[,2][!tree$edge[,2]%in%1:length(tree$tip.label)]
    internal_node_branch_lengths=sapply(internal_nodes,function(node) tree$edge.length[tree$edge[,2]==node])
    internal_node_total_clade_branch_lengths=sapply(internal_nodes, function(node) {get_total_clade_branch_lengths(node = node,tree)})
    somatic_revision_probs=internal_node_branch_lengths*internal_node_total_clade_branch_lengths
    
    for(i in 1:nsim) {
      if(i%%1000==0){print(i)}
      
      #Choose the lesion node for the mutation that will undergo a somatic reversion
      mutation_node=base::sample(x = internal_nodes,size = 1,replace = F,prob = somatic_revision_probs)
      
      #Now choose the branch where somatic reversion will occur: ~branch_length
      clade_nodes=get_all_node_children(mutation_node,tree)
      reversion_node=sample(clade_nodes,size = 1,prob = sapply(clade_nodes,function(node) tree$edge.length[tree$edge[,2]==node]/get_total_clade_branch_lengths(mutation_node,tree)))
      pos_samples=getTips(tree,mutation_node)[!getTips(tree,mutation_node)%in%getTips(tree,reversion_node)]
      neg_samples=tree$tip.label[!tree$tip.label%in%pos_samples]
      sim$NR[i,]<-rpois(n=length(tree$tip.label),lambda = mean_depth)
      ...
      sim$sim_nodes[i,]<-c(mutation_node,reversion_node)
      if(reversion_node%in%get_node_children(mutation_node,tree)) {sim$isPVV[i]<-F} else {sim$isPVV[i]<-T}
    }
    
    sim$isPVV[is.na(sim$isPVV)]<-F
    print(paste("There are",sum(sim$isPVV,na.rm = T),"detectable somatic reversion events in this simulation"))
    
    sim$NV<-sim$NV[sim$isPVV,]
    sim$NR<-sim$NR[sim$isPVV,]
    sim$sim_nodes<-sim$sim_nodes[sim$isPVV,]
    sim$details<-sim$details[sim$isPVV,]
    ...
    include_ancestral_tip=ifelse(dataset%in%c("MF","NW"),T,F)
    ...
    if(sum(filter_res)>0) {
      ...
      poor_fit_list=lapply(mutations_to_test[filter_res],function(mut) {
        df=data.frame(mut_ref1=mut,
                      node=sim$details$node[sim$details$mut_ref==mut],
                      Sample_ID=sample,
                      lesion_node=NA,
                      lesion_timing=NA,
    ...
    poor_fit_df$result=ifelse(is.na(poor_fit_df$lesion_node),"FAIL","PASS")
    table(poor_fit_df$result)
    ...
    poor_fit_df$by_chance_prob=sapply(1:nrow(poor_fit_df),function(i) {
      ...
      if(any(pure_subclades=="Not PVV")){stop(return(NA))}
      pure_negative=pure_subclades[which(names(pure_subclades)=="pure_negative")]
      by_chance=prod(sapply(c(lesion_node,unlist(pure_negative)),function(node) tree$edge.length[tree$edge[,2]==node]/sum(tree$edge.length)))
      score=-log10(by_chance)
      return(score)
    })
    write.table(poor_fit_df,file=paste0(simulation_output_dir,"/",sample,"_poor_fit_reversion_simulation.tsv"),sep="\t",quote=F,row.names = F)
    ...
    p1<-poor_fit_df%>%
      group_by(result)%>%
      summarise(n=n())%>%
      bind_rows(data.frame(result=c("Somatic reversion on daughter branch","Failed PVV assessment"),n=c(nsim-length(mutations_to_test),n_failed_PVV_assessment)))%>%
      mutate(prop=n/sum(n),result=factor(result,levels=c("Somatic reversion on daughter branch","Failed PVV assessment","FAIL","PASS")))%>%
      ...
      my_theme+
      labs(title="Outcome of simulated somatic reversion events")
    ...
    p3<-poor_fit_df%>%
      ...
      geom_point(data=PVV_data,aes(col=PVV_pos_clade_phasing),size=3)+
    ...
    p4<-poor_fit_df%>%
      ...
      stat_density_2d(aes(fill = ..level..), geom = "polygon", colour="white",size=0.1)+
      my_theme+
      geom_jitter(data=PVV_data,aes(col=PVV_pos_clade_phasing),size=3,height=0.1)+
      scale_color_manual(name = "PVV_pos_clade_phasing",values = myColors)+
      scale_fill_continuous(guide='none')
    
    PVV_data_probs<-PVV_data%>%mutate(result=factor(result,levels=c("FAIL","PASS")))%>%count(result)%>%complete(result,fill=list(n=0))%>%mutate(prop=n/sum(n),data_type="data")
    PVV_sim_probs<-poor_fit_df%>%filter(result!="Not PVV")%>%mutate(result=factor(result,levels=c("FAIL","PASS")))%>%count(result)%>%complete(result,fill=list(n=0))%>%mutate(prop=n/sum(n),data_type="simulation")
    
    p5<-bind_rows(PVV_data_probs,PVV_sim_probs)%>%
      ggplot(aes(x=data_type,y=prop,fill=result))+
    ...
    ggsave(filename=paste0("output2/PVV_vs_sim_plots/",sample,"_PVV_vs_reversion_sim_plots.pdf"),plot=plot_comb,device="pdf",width=8,height=6)
  })
})
